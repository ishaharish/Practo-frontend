...
export class PatientDetailsComponent implements OnInit {
  ...
  fetchPatientTimeline() {
    ...
    this.http.get<any>(`/api/doctors/dashboard/patients/${this.patientId}`)
      .subscribe({
        next: (res) => {
  ...
  saveClinicalRecord() {
    ...
    this.http.post(`/api/appointments/${latestApptId}/medical-record`, payload)
      .subscribe({
        next: () => {
  ...
  deletePatient() {
    if (confirm('Are you sure you want to delete this patient and all of their historical records? This cannot be undone.')) {
      this.http.delete(`/api/doctors/dashboard/patients/${this.patientId}`)
        .subscribe({
          next: () => {
  ...
  scheduleNextVisit() {
    ...
    this.http.post('/api/ray/appointments/next-visit', payload)
      .subscribe({
        next: () => {
